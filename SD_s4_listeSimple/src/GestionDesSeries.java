...
public class GestionDesSeries {
	...
	private static void rechercherSerie() {
		...
		scanner.nextLine(); // pour nettoyer un éventuel \n précédent
		String nom = scanner.nextLine();

		try {
			int numero = classe.numeroSerie(nom); // méthode qui renvoie le numéro ou -1
			if (numero == -1) {
				System.out.println("L'étudiant " + nom + " n'appartient à aucune série.");
			} else {
				System.out.println("L'étudiant " + nom + " est dans la série n°" + numero + ".");
			}
