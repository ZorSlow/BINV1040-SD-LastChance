...
public class ListeRecCaracteres {
	...
	private boolean contient(NoeudCaractere noeud , char caratereRecherche){
		if (noeud == null)
			return false;
		if (noeud.caractere == caratereRecherche)
			return true;
		return contient(noeud.suivant,caratereRecherche);
	...
	public void remplacerTout(NoeudCaractere noeud,char caractereARemplacer, char nouveauCaractere){
		if (noeud == null)
			return;
		if (noeud.caractere == caractereARemplacer)
			noeud.caractere = nouveauCaractere;
	...
	private char max(NoeudCaractere noeud) {
		...
		if (noeud.caractere > maxChar)
			maxChar = noeud.caractere;
		return maxChar;
	}
	...
	public ArrayList<Character> enArrayList(){
		ArrayList listCarateres = new ArrayList<>();
		enArrayList(tete,listCarateres);
		return  listCarateres;
	...
	}	private void enArrayList(NoeudCaractere noeud, ArrayList<Character> listCarateres){
		if (noeud == null) {
			return ;
		}
		listCarateres.add(noeud.caractere);
		enArrayList(noeud.suivant,listCarateres);
	}
